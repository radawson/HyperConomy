...
public class UpdateYML {
	...
	public UpdateYML(HyperConomy hc) {
		...
		cfg.setDefault("economy-plugin.hook-internal-economy-into-serviceio", false);
		cfg.setDefault("economy-plugin.starting-player-account-balance", 0);
		
		// Currency system configuration
		cfg.setDefault("currency.type", "ITEM");
		cfg.setDefault("currency.item.material", "GOLD_NUGGET");
		cfg.setDefault("currency.item.display-name", "Gold Nugget");
		cfg.setDefault("currency.item.custom-model-data", null);
		cfg.setDefault("currency.balance-mode", false);
